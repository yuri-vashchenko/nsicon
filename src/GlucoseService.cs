...
namespace NsIcon
{
    ...
    class GlucoseService
    {
        ...
        public GlucoseService()
        {
            ...
            this.timer = new System.Timers.Timer(NsIcon.Properties.Settings.Default.updateInterval); // Interval in ms.
            ...
            if (!NsIcon.Properties.Settings.Default.disclaimerAgreed)
            {
                this.timer.Stop();
        ...
        {
            if (String.IsNullOrEmpty(NsIcon.Properties.Settings.Default.serverUrl))
            {
                OnStatusChange(true, "No Nightscout server url set-up.");
                return;
            }

            if (String.IsNullOrEmpty(NsIcon.Properties.Settings.Default.apiSecretSha1))
            {
            ...
            if (jsonObjEntry.ContainsKey("sysTime"))
            {
                ...
                if (NsIcon.Properties.Settings.Default.timezone == "UTC") { 
                    ...
                    try
                    {
                        TimeZoneInfo tzi = TimeZoneInfo.FindSystemTimeZoneById(NsIcon.Properties.Settings.Default.timezone);
        ...
        private string DownloadNightscoutEntries()
        {
            ...
            System.Net.ServicePointManager.DnsRefreshTimeout = NsIcon.Properties.Settings.Default.durationDNSCache;
            ...
            string urlEntries = NsIcon.Properties.Settings.Default.serverUrl + "/api/v1/entries.json?count=2&find[date][$gte]=" + milisecondsSinceEpoch.ToString();
            ...
            request.Timeout = NsIcon.Properties.Settings.Default.networkTimeout;
            request.Proxy = null;
            // Authentication
            request.Headers["API-SECRET"] = NsIcon.Properties.Settings.Default.apiSecretSha1.ToLowerInvariant();
        ...
        private long GetMsSinceEpochMinsGlucoseValidDuration()
        {
            ...
            long secondsEpochWGlucoseValid = (long)Math.Round(t.TotalSeconds, 0, MidpointRounding.AwayFromZero) - NsIcon.Properties.Settings.Default.secondsGlucoseValueValid;
